...
/obj/item/weapon/secstorage/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/weapon/card/emag)) && (src.locked == 1) && (!src.emagged))
		emagged = 1
		src.overlays += image('icons/Uristqwerty/food_processor_placeholder.dmi', icon_sparking)
		sleep(6)
		src.overlays = null
		overlays += image('icons/Uristqwerty/food_processor_placeholder.dmi', icon_locking)
		locked = 0
	...
	if (href_list["type"])
		if (href_list["type"] == "E")
			if ((src.l_set == 0) && (length(src.code) == 5) && (!src.l_setshort) && (src.code != "ERROR"))
				...
				src.locked = 0
				src.overlays = null
				overlays += image('icons/Uristqwerty/food_processor_placeholder.dmi', icon_open)
				src.code = null
			else
				src.code = "ERROR"
		else
