...
Doing this because FindTurfs() isn't even used
				for(var/atom/U in OM.FindTurfs() )
					...
					if(dirn == 1)
						O.overlayss += image('icons//debug_connect.dmi', OM.airdir==1?"up":"fup")
					else if(dirn == 2)
						O.overlayss += image('icons//debug_connect.dmi', OM.airdir==2?"dn":"fdn")
					else if(dirn == 4)
						O.overlayss += image('icons//debug_connect.dmi', OM.airdir==4?"rt":"frt")
					else if(dirn == 8)
						O.overlayss += image('icons//debug_connect.dmi', OM.airdir==8?"lf":"flf")
*/
			else
				...
				if(T.airN)
					O.overlayss += image('icons//debug_connect.dmi', T.airdir==1?"up":"fup")

				if(T.airS)
					O.overlayss += image('icons//debug_connect.dmi', T.airdir==2?"dn":"fdn")

				if(T.airW)
					O.overlayss += image('icons//debug_connect.dmi', T.airdir==8?"lf":"flf")

				if(T.airE)
					O.overlayss += image('icons//debug_connect.dmi', T.airdir==4?"rt":"frt")


				if(T.condN)
					O.overlayss += image('icons//debug_connect.dmi', T.condN == 1?"yup":"rup")

				if(T.condS)
					O.overlayss += image('icons//debug_connect.dmi', T.condS == 1?"ydn":"rdn")

				if(T.condE)
					O.overlayss += image('icons//debug_connect.dmi', T.condE == 1?"yrt":"rrt")

				if(T.condW)
					O.overlayss += image('icons//debug_connect.dmi', T.condW == 1?"ylf":"rlf")
	else
		alert("Debugging off")
		return
...
/proc/numbericon(var/tn as text,var/s = 0)
	if(Debug)
		var/image/I = image('icons//debug_connect.dmi', "blank")
		...
		for(var/d = 1 to lentext(tn))
			...
			var/image/ID = image('icons//debug_connect.dmi', char)
